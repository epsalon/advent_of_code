...
sub nf_rev {
  ...
  my @hallway = split('', $1);
  my @can_move;
  ROOM: for my $i (0..@p/$roomsize-1) {
    my $id = $ROOM_IDS[$i];
    for my $e (0..$roomsize-1) {
      $e = $roomsize-1-$e;
      next ROOM if ($p[$roomsize*$i+$e] ne $id);
      if (!$e || !($p[$roomsize*$i+$e-1])) {
        my @np = @p;
        $np[$roomsize*$i+$e] = 0;
        push @can_move, [$id, $i, $e + 1, join('',@np)];
      }
    }
  }
  ...
  for my $i (0..$#hallway) {
    ...
    for my $d ($min..$max) {
      ...
      for my $e (0..$roomsize-1) {
        $e = $roomsize - 1 - $e;
        unless ($p[$roomsize*$dest+$e]) {
          $extra_cost=$e + 1;
          last;
        }
      }
      if ($extra_cost) {
        my $steps = abs($i-$d) + $extra_cost;
        my $cost = $steps * $COSTS{$id};
        my @nh = @hallway;
        $nh[$i] = 0;
        my @np = @p;
...
my @n = split('', 'AABBCCDD');
for my $p (map { join '', @$_ } permute(@n)) {
  #$p =~ s/(.)(..)(..)(..)(.)/$1DD$2CB$3BA$4AC$5/o;
  $END{"$hallway|$p"}=1;
}

$|=1;

my @ret = astar(end_state,'',\&nf_rev);
out(shift @ret);
