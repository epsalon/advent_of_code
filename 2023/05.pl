...
while (<>) {
  ...
  unless ($_) {
    ...
    YY: while (@s) {
      my ($v, $vd) = @{shift @s};
      say "considering ($v,$vd)";
      #say "v=$v vd=$vd";
      for my $y (@y) {
        my ($d,$s,$l) = @$y;
        #say "v=$v vd=$vd d=$d s=$s l=$l";
        next if ($v + $vd <= $s);
        next if ($s + $l <= $v);
        if ($v < $s) {
          push @s, [$v,$s-$v];
        ...
        if ($s + $l < $v + $vd) {
          push @s, [$s + $l, $v + $vd - $s - $l];
          say "retaining [".($s + $l).", ".($v + $vd - $s - $l)."]";
        }
        my $start = max($v,$s) - $s + $d;
        my $end =  min($v + $vd,$s + $l) - $s + $d;
        push @o, [$start, $end - $start];
        say "converting [$start, ".($end-$start)."]";
        next YY;
      }
      say "retaining unchanged [$v, $vd]";
      push @o, [$v, $vd];
    };
    @s=@o;
    out (\@s);
